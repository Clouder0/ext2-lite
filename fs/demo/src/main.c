...
static void *demo_mount(struct fuse_conn_info *conn_info)
{
    ...
    /* 填充super信息 */
    if(ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_IO_SZ, &super.sz_io)) {
        printf("get super device io size failed\n");
        exit(-1);
    }
    ddriver_ioctl(super.driver_fd, IOC_REQ_DEVICE_SIZE, &super.sz_disk);
    super.sz_blks = super.sz_io * 2;
    printf("Done!\n");
    printf("sz blks %d, sz io %d, sz disk %d\n", super.sz_blks, super.sz_io, super.sz_disk);
...
static int demo_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi)
{
    ...
    char *mybuf = (char*)malloc(super.sz_io);
    ddriver_read(super.driver_fd, mybuf, super.sz_io);

    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    struct demo_dentry *d = (struct demo_dentry *)mybuf;


    /* TODO: 填充filename */
    strcpy(filename, d->fname);
